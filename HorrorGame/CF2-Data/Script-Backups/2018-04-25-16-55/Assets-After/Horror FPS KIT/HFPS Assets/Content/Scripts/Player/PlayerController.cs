...
public class PlayerController : MonoBehaviour
{
	...
	void SetKeys()
	{
		...
		JumpKey = parser.ParseType<KeyCode>(inputManager.GetInput("Jump"));
		RunKey = parser.ParseType<KeyCode>(inputManager.GetInput("Run"));
		CrouchKey = parser.ParseType<KeyCode>(inputManager.GetInput("Crouch"));
		isSet = true;
	}
    ...
    void Update()
    {
		...
		if (controllable) {
			if (ControlFreak2.CF2Input.GetKey (ForwardKey)) {
				GetKeyAxis ("Y", "fw", 1);
			} else {
				GetKeyAxis ("Y", "fw", 0);
			}
		
			if (ControlFreak2.CF2Input.GetKey (BackwardKey)) {
				GetKeyAxis ("Y", "bw", -1);
			} else {
				GetKeyAxis ("Y", "bw", 0);
			}
		
			if (ControlFreak2.CF2Input.GetKey (LeftKey)) {
				GetKeyAxis ("X", "l", -1);
			} else {
				GetKeyAxis ("X", "l", 0);
			}
		
			if (ControlFreak2.CF2Input.GetKey (RightKey)) {
				GetKeyAxis ("X", "r", 1);
			} else {
				GetKeyAxis ("X", "r", 0);
			}
