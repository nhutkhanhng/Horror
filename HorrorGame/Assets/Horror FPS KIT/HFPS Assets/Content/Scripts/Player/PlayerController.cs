...
public class PlayerController : MonoBehaviour
{
    ...
    public float normalFDTreshold = 8;
    public float crouchFDTreshold = 4;
	...
	public DelayEffect DelayEffect;
	...
	void Awake()
	{
        ...
        switcher = scriptManager.itemSwitcher;
	...
	void SetKeys()
	{
		ForwardKey = parser.ParseType<KeyCode>(inputManager.GetInput("Forward"));
		BackwardKey = parser.ParseType<KeyCode>(inputManager.GetInput("Backward"));
		LeftKey = parser.ParseType<KeyCode>(inputManager.GetInput("Left"));
		RightKey = parser.ParseType<KeyCode>(inputManager.GetInput("Right"));
		RunKey = parser.ParseType<KeyCode>(inputManager.GetInput("Run"));
		isSet = true;
	}
				...
				switch(dir)
				{
					case "fw":
						switch(axis)
						{
							case 1:
								if(inputY < 1){
									inputY += Time.deltaTime/3 * startWalkSpeed;
									mfw = true;
								}else{
									inputY = 1f;
								}
							...
							case 0:
								...
								{
									if(inputY > 0){
										inputY -= Time.deltaTime * 3 * stopWalkSpeed;
									}else{
										inputY = 0f;
										mfw = false;
									}
				...
				switch(dir)
				{
					case "r":
						switch(axis)
						{
							case 1:
								if(inputX < 1){
									inputX += Time.deltaTime/3 * startWalkSpeed;
									mr = true;
								}else{
									inputX = 1f;
								}
    ...
    void Update()
    {
	    ...
	    if (HFPS_GameManager.Instance.DownExamineUI.activeSelf)
	    {
		    ControllerInputX = 0;
		    ControllerInputY = 0;
	    }
        ...
        //inputX = ControllerInputX;
        //inputY = ControllerInputY;
        ...
        if (grounded)
        {
            ...
            if (canRun && state == 0)
            {
                if (ControlFreak2.CF2Input.GetKey(RunKey) && ControlFreak2.CF2Input.GetKey(ForwardKey) && !ControlFreak2.CF2Input.GetKey(ZoomKey))
                {
                    run = true;
                }
                else
            ...
            {
                if (state == 2)
                    fallingDamageThreshold = crouchFDTreshold;
                else
            ...
            else
            {
				if (state == 0) {
					if (run)
						speed = runSpeed;
					else if (ControlFreak2.CF2Input.GetKey (ZoomKey)) {
						speed = crouchSpeed;
					} else {
						speed = walkSpeed;
					}
				...
				if (!ControlFreak2.CF2Input.GetKey (JumpKey)) {
					jumpTimer++;
		...
		if (ControlFreak2.CF2Input.GetKeyDown(CrouchKey))
        {
            CheckDistance();
	...
	void localTouch(InputTouch touch)
	{
		...
		if (!HFPS_GameManager.Instance.PaperTextUI.activeSelf)
		{
