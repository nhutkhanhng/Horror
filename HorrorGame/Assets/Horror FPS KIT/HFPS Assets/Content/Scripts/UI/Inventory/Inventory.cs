...
public class Inventory : MonoBehaviour, IPointerClickHandler
{
	...
	void Start () {
		...
		for (int i = 0; i < slotAmout; i++) {
			slots.Add(Instantiate(inventorySlot,SlotsPanel.transform));
	...
	void Update()
	{
		...
		if (ControlFreak2.CF2Input.GetKeyDown(SwitchKey) && !isSwitch)
		{
			SwitchWeapon();
		}
		else if (isSwitch)
	...
	public void SwitchWeapon()
	{
		...
		switch (switcher.currentItem)
		{
			case 0:
				if (GetItemAmount(CandleID) > 0)
					switcher.selectItem(1);
				else if (GetItemAmount(GlockID) >= 0)
					switcher.selectItem(2);
				break;
			case 1:
				if (GetItemAmount(GlockID) >= 0)
					switcher.selectItem(2);
				else if (GetItemAmount(FlashID) > 0)
					switcher.selectItem(0);
				break;
			case 2:
				if (GetItemAmount(FlashID) > 0)
					switcher.selectItem(0);
				else if (GetItemAmount(CandleID) > 0)
					switcher.selectItem(1);
				break;
		}
