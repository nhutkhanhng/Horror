...
public class DynamicObjectManager : MonoBehaviour {
	...
	void Update () {
		...
		{
			if (dynamic.dynamicType == DynamicObject.dynamic.Valve)
			{
		...
		Ray playerAim = new Ray(InteractManager.Instance.laserPointer.transform.position,InteractManager.Instance.laserPointer.transform.forward);
		RaycastHit hit;
