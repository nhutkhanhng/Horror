...
public class DynamicObject : MonoBehaviour {
    ...
    void Update() {
        ...
        else if (dynamicType == dynamic.Valve)
        {
            ...
            if (isValve)
            {
                if (!isInteractedValve)
                {
                    isInteractedValve = true;
                    ValvePass();
                }
                else
                {
    ...
    void ValvePass()
    {
        if (!(rotateValue >= 1f))
        {
            HFPS_GameManager.Instance.NewValveSlider(rotateValue, valveRotateTime);
        }
    }
    ...
    {
        if(!(rotateValue >= 1f)){
            float z = valveRotateSpeed;
            Vector3 rotation = new Vector3(0, 0, -z);
            gameObject.transform.Rotate(rotation);
            rotateValue = HFPS_GameManager.Instance.ValveSlider.value;
        }
        else
        {
            isValve = false;
            HFPS_GameManager.Instance.DisableValveSlider();
        }
    }
}
