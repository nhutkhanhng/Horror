...
public Rect GetScreenEmuRect()
	{ return GetScreenEmuRect(false); }		///< When set to true, returned rect will be relative to bottom-left corner of the screen (as used by Camera.pixelRect). When false, rect will be relative to top-left corner (as used by GUI).  
...
*/
			float idealToScreenX = (boxSize.x / idealContentSizeX); 
			float idealToScreenY = (boxSize.y / idealContentSizeY); 
			
			if (this.alwaysFill)
				{
				// Try to maintain margins if we have the space...
				
				if (boxSize.x > idealContentSizeX)
					idealToScreenX = Mathf.Max(idealContentSizeX, (boxSize.x - maxMarginX)) / idealContentSizeX; 
				if (boxSize.y > idealContentSizeY)
					idealToScreenY = Mathf.Max(idealContentSizeY, (boxSize.y - maxMarginY)) / idealContentSizeY; 
				}		
			
			
			this.contentSizeScale = (Mathf.Min(idealToScreenX, idealToScreenY));
		
			if (!this.alwaysFill)
				{
				idealToScreenX = Mathf.Clamp01(idealToScreenX);
				idealToScreenY = Mathf.Clamp01(idealToScreenY);
				this.contentSizeScale = Mathf.Clamp01(this.contentSizeScale);
				}
			
			if (this.allowNonuniformScale)
				this.contentPosScale = new Vector2((idealToScreenX), (idealToScreenY));
			else
				this.contentPosScale = new Vector2(this.contentSizeScale, this.contentSizeScale);
